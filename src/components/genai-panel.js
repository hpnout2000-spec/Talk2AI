...
function renderAssistantBubble(entry, bubbleEl, { cursor = false, preemptiveWorking = false, streaming = false, animate = false } = {}) {
  ...
  const renderSectionHtml = (sectionText, isProcessSection = false) => {
    ...
    suggestData.forEach((data, idx) => {
      ...
      sHtml = sHtml.split(token).join(spanHtml);
    });
    ...
    if (entry.tools && entry.tools.length > 0) {
      entry.tools.forEach((tool, idx) => {
                ...
                <div class="genai-inline-tool genai-tool-pending" id="genai-tool-${idx}" style="margin: var(--space-2) 0; width: 100%; display: flex; align-items: center; gap: var(--space-3); line-height: 1.2;">
                  ...
                  <div style="display: flex; gap: 8px; margin-left: auto;">
                    ...
                    <button id="deny-tool-${idx}" class="continuation-option-btn" 
                            style="margin: 0; padding: 4px 14px; background: transparent; border: 1px solid var(--border-light, rgba(255,255,255,0.2)); color: var(--text-muted); border-radius: 100px; cursor: pointer; font-size: 0.8em; opacity: 1; transform: none; animation: none; transition: all 0.2s ease;">
                      Deny
                    </button>
                  </div>
                </div>
        ...
        sHtml = sHtml.split('<p>' + marker + '</p>').join(marker);
        sHtml = sHtml.split('<p>\n' + marker + '\n</p>').join(marker);
        sHtml = sHtml.split('<p>\r\n' + marker + '\r\n</p>').join(marker);
        sHtml = sHtml.replace(new RegExp('<p>\\s*' + marker.replace(/\[/g, '\\[').replace(/\]/g, '\\]') + '\\s*<\\/p>', 'g'), marker);
        
        sHtml = sHtml.split(marker).join(badgeHtml);
      });
    }
  ...
  const hasPendingTool = entry.tools && entry.tools.some(t => t.state === 'awaiting_approval');
  const isGeneratingOrWorking = streaming || hasWorkingTool || hasPendingTool;

  let lastMarkerEnd = -1;
  if (entry.tools && entry.tools.length > 0) {
    for (let i = 0; i < entry.tools.length; i++) {
      const marker = `[[GENAI_TOOL_${i}]]`;
      const idx = processedText.lastIndexOf(marker);
      if (idx !== -1 && idx + marker.length > lastMarkerEnd) {
        lastMarkerEnd = idx + marker.length;
      }
    }
  }
  if (entry.thinking_blocks && entry.thinking_blocks.length > 0) {
  ...
  try {
    ...
    if (!autoApprove) {
      ...
      try {
        ...
        console.error('Render bubble failed in handleMultipleActions', e);
      }
    ...
    if (bubbleEl) {
      bubbleEl.innerHTML += `<div style="color:red; font-weight:bold; margin-top:8px;">ERROR: ${err.message}<br>${err.stack}</div>`;
      scrollToBottom();
    }
    isGenerating = false;
    finishGeneration();
