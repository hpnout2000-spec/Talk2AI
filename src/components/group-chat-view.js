...
export function initGroupChatView() {
  ...
  headerResponseMode = document.getElementById('group-response-mode');
  ...
  headerResponseMode?.addEventListener('change', async (e) => {
    const group = groupChatStore.getGroupById(groupChatStore.getActiveGroupId());
    if (!group) return;
    await groupChatStore.updateGroupResponseMode(group.id, e.target.value);
...
async function handleSend() {
  ...
  const groupId = groupChatStore.getActiveGroupId();
  const group = groupChatStore.getGroupById(groupId);
  if (!group) { showToast('No group selected', 'error'); return; }
  
  const characters = (group.character_ids || [])
    .map(id => characterStore.getById(id))
    .filter(Boolean);

  if (characters.length === 0) {
    showToast('Add characters to this group first', 'error');
    return;
  }
...
async function pickCharacterWithAI(group, characters, session) {
  try {
    ...
    const history = session.messages.slice(-6).map(m => {
      const name = m.role === 'user' ? 'User' : (characters.find(c => c.id === m.character_id)?.name || 'AI');
    ...
    const prompt = [
      {
        role: 'system',
        content: `Conversation history:\n${history}\n\nAvailable characters: ${charNames}. Who should respond next? Reply ONLY with the character's name.`
      }
    ];

    const result = await api.chatCompletion(prompt, { max_tokens: 15, temperature: 0.1 });
    const picked = result.trim().replace(/[".]/g, '');
    const found = characters.find(c => c.name.toLowerCase() === picked.toLowerCase());
  ...
  try {
    await api.streamChat(
      ...
      async () => {
        ...
        const newEl = appendGroupMessage(finalMsg, false);
...
function buildGroupApiMessages(respondingChar, allCharacters, session) {
  const settings = settingsStore.get();
  const userName = settings.user_name || 'User';
  const messages = [];

  const otherChars = allCharacters.filter(c => c.id !== respondingChar.id);
  let sys = respondingChar.system_prompt || [
    respondingChar.description,
    respondingChar.personality,
    respondingChar.scenario
  ].filter(Boolean).join('\n\n') || `You are ${respondingChar.name}.`;

  sys = sys.replace(/\{\{user\}\}/gi, userName).replace(/\{\{char\}\}/gi, respondingChar.name);
  sys += `\n\n[GROUP CHAT]\nYou are ${respondingChar.name}. Others: ${otherChars.map(c => c.name).join(', ')}. ONLY speak as yourself.`;
  ...
  for (const m of session.messages) {
    if (m.role === 'user') messages.push({ role: 'user', content: m.content });
      ...
      if (m.character_id === respondingChar.id) messages.push({ role: 'assistant', content: m.content });
      else {
        const c = allCharacters.find(x => x.id === m.character_id);
        messages.push({ role: 'system', content: `[${c?.name || 'AI'} said]: ${m.content}` });
      }
    }
  }
  return messages;
...
function appendGroupMessage(msg, isStreaming = false) {
  ...
  const avatarHtml = isUser 
    ...
    : (char?.avatar ? `<img src="${char.avatar}">` : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>');
  ...
  const meta = !isStreaming ? `
    <div class="group-msg-meta">
      ...
      <div class="message-actions">
        <button class="btn-regenerate ${isUser ? 'hidden' : ''}" title="Regen"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 4v6h6M23 20v-6h-6"/><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"/></svg></button>
        <button class="btn-edit-msg" title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
        <button class="btn-copy" title="Copy"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>
        <button class="btn-ai-comment ${isUser ? 'hidden' : ''}" title="Comment"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/><path d="M12 7l1.5 3 3.5.5-2.5 2.5.5 3.5-3-1.5-3 1.5.5-3.5-2.5-2.5 3.5-.5z"/></svg></button>
        <button class="btn-delete-msg delete" title="Del"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </div>
    </div>
  ` : '';
  ...
  el.innerHTML = `
    ...
    <div class="group-msg-body">
      ...
      <div class="group-msg-bubble"><div class="group-msg-text">${isStreaming ? '<span class="streaming-cursor"></span>' : contentHtml}</div></div>
      ${meta}
    </div>
  `;
  ...
  if (!isStreaming) {
    ...
    el.querySelector('.btn-delete-msg')?.addEventListener('click', async () => {
      if (await showConfirm('Delete?')) { groupChatStore.deleteMessage(msg.id); el.remove(); groupChatStore.saveGroups(); }
    });
    el.querySelector('.btn-edit-msg')?.addEventListener('click', () => {
      const t = prompt('Edit:', msg.content);
      if (t !== null) { msg.content = t; el.querySelector('.group-msg-text').innerHTML = renderMarkdown(t); groupChatStore.saveGroups(); }
    });
    el.querySelector('.btn-ai-comment')?.addEventListener('click', () => {
      if (window.requestAiComment && char) window.requestAiComment(msg, char);
    });
...
function scrollGroupToBottom() {
  requestAnimationFrame(() => { if (messagesContainer) messagesContainer.scrollTop = messagesContainer.scrollHeight; });
}
...
function renderHeaderModeUI(group) {
  ...
  if (group && headerResponseMode) headerResponseMode.value = group.response_mode || 'round_robin';
}
...
function renderMembersPopover() {
  ...
  membersPopover.innerHTML = `
    <div class="group-popover-header"><span>Members</span><button id="btn-close-group-popover" class="btn-icon small"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div>
    <div class="group-popover-members">${all.map(c => `<div class="group-popover-member"><div class="group-popover-avatar">${c.avatar ? `<img src="${c.avatar}">` : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>'}</div><span class="group-popover-name">${escapeHtml(c.name)}</span><label class="toggle-switch small" style="margin-left:auto;"><input type="checkbox" class="member-toggle" data-char-id="${c.id}" ${ids.has(c.id) ? 'checked' : ''}><span class="toggle-slider"></span></label></div>`).join('')}</div>`;
